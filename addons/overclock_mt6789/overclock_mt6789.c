...
static unsigned int cpu_c1_rep_cpu = 4;
static unsigned int cpu_c2_rep_cpu = 6;
module_param(cpu_c0_rep_cpu, uint, 0444);
module_param(cpu_c1_rep_cpu, uint, 0444);
module_param(cpu_c2_rep_cpu, uint, 0444);
MODULE_PARM_DESC(cpu_c0_rep_cpu, "Representative CPU for 4-core A55 policy (CPU0-3)");
MODULE_PARM_DESC(cpu_c1_rep_cpu, "Representative CPU for 2-core A55 policy (CPU3-5)");
MODULE_PARM_DESC(cpu_c2_rep_cpu, "Representative CPU for 2-core A76 policy (CPU6-7)");

#define MAX_OC_PERCENT_OVER_STOCK  60

#define MAX_OC_ABSOLUTE_KHZ  2600000U

static unsigned int cpu_c0_target_khz = 0;
static unsigned int cpu_c1_target_khz = 0;
static unsigned int cpu_c2_target_khz = 0;
...
static unsigned int g_c1_requested_min_khz;
static unsigned int g_c2_requested_min_khz;
static int          cpu_oc_apply      = 0;
module_param(cpu_c0_target_khz, uint, 0644);
MODULE_PARM_DESC(cpu_c0_target_khz, "Cluster 0 CPU0-3 target max frequency KHz (0=stock)");
module_param(cpu_c1_target_khz, uint, 0644);
MODULE_PARM_DESC(cpu_c1_target_khz, "Cluster 1 CPU3-5 target max frequency KHz (0=stock)");
module_param(cpu_c2_target_khz, uint, 0644);
MODULE_PARM_DESC(cpu_c2_target_khz, "Cluster 2 CPU6-7 target max frequency KHz (0=stock)");
...
static unsigned int g_c0_orig_khz, g_c1_orig_khz, g_c2_orig_khz;
static unsigned int g_a55_hw_orig_khz;
static bool g_a55_hw_have_orig;
static bool g_c0_have_orig, g_c1_have_orig, g_c2_have_orig;
...
static bool policy_is_virtual_a55(struct cpufreq_mtk_mirror *c)
{
	return c && c->shared && c->shared != c && c->virtual_id == 1;
}
			       ...
			       bool write_hw)
{
	...
	hw = policy_is_virtual_a55(c) ? c->shared : c;

	if (!g_a55_hw_have_orig && (rep_cpu == cpu_c0_rep_cpu || policy_is_virtual_a55(c))) {
		g_a55_hw_orig_khz = hw->table[0].frequency;
		g_a55_hw_have_orig = true;
	}
	...
	if (write_hw) {
		if (policy_is_virtual_a55(c) && target_khz == 0)
			use_khz = hw->table[0].frequency;

		raw = readl_relaxed(hw->reg_bases[REG_FREQ_LUT_TABLE] + (0 * LUT_ROW_SIZE));
		raw = (raw & ~LUT_FREQ) | FIELD_PREP(LUT_FREQ, use_khz / 1000);
		writel_relaxed(raw, hw->reg_bases[REG_FREQ_LUT_TABLE] + (0 * LUT_ROW_SIZE));
	}
	...
	pr_info("oc_mt6789: C%u policy cpu%u max=%u KHz, hardware LUT owner cpu%u\n",
		(rep_cpu == cpu_c0_rep_cpu) ? 0 : ((rep_cpu == cpu_c1_rep_cpu) ? 1 : 2),
		rep_cpu, use_khz, policy_is_virtual_a55(c) ? cpu_c0_rep_cpu : rep_cpu);

	cpufreq_cpu_put(policy);
	return 0;
}

static unsigned int a55_hw_target_khz(void)
{
	unsigned int stock = g_a55_hw_have_orig ? g_a55_hw_orig_khz : 2000000U;
	unsigned int c0 = cpu_c0_target_khz ? cpu_c0_target_khz : stock;
	unsigned int c1 = cpu_c1_target_khz ? cpu_c1_target_khz : stock;

	return max(c0, c1);
}

static int apply_shared_a55_hw(void)
{
	struct cpufreq_policy *policy;
	struct cpufreq_mtk_mirror *c;
	struct cpufreq_mtk_mirror *hw;
	...
	policy = cpufreq_cpu_get(cpu_c0_rep_cpu);
	if (!policy)
		return -ENODEV;
	if (!policy->driver_data || !policy->freq_table) {
		cpufreq_cpu_put(policy);
	...
	raw = (raw & ~LUT_FREQ) | FIELD_PREP(LUT_FREQ, target / 1000);
	writel_relaxed(raw, hw->reg_bases[REG_FREQ_LUT_TABLE]);
	hw->table[0].frequency = target;
...
static int cpu_oc_apply_set(const char *val, const struct kernel_param *kp)
{
	...
	if (cpu_c0_target_khz || g_c0_have_orig)
		ret0 = patch_cluster_idx0(cpu_c0_rep_cpu, cpu_c0_target_khz,
					 &g_c0_orig_khz, &g_c0_have_orig, false);
	if (cpu_c1_target_khz || g_c1_have_orig)
		ret1 = patch_cluster_idx0(cpu_c1_rep_cpu, cpu_c1_target_khz,
					 &g_c1_orig_khz, &g_c1_have_orig, false);
	if (cpu_c2_target_khz || g_c2_have_orig)
		ret2 = patch_cluster_idx0(cpu_c2_rep_cpu, cpu_c2_target_khz,
					 &g_c2_orig_khz, &g_c2_have_orig, true);

	if (!ret0 && !ret1 && (cpu_c0_target_khz || cpu_c1_target_khz ||
		g_c0_have_orig || g_c1_have_orig)) {
		int hw_ret = apply_shared_a55_hw();
		if (hw_ret && !ret0)
			ret0 = hw_ret;
	}

	if (ret0 == -ERANGE || ret1 == -ERANGE || ret2 == -ERANGE)
		snprintf(cpu_oc_result, sizeof(cpu_oc_result),
			 "FAIL: target exceeds safety cap (c0=%u c1=%u c2=%u KHz)",
			 cpu_c0_target_khz, cpu_c1_target_khz, cpu_c2_target_khz);
	else if (ret0 == -ETIMEDOUT || ret1 == -ETIMEDOUT || ret2 == -ETIMEDOUT)
		snprintf(cpu_oc_result, sizeof(cpu_oc_result),
			 "FAIL: one cluster did not leave idx0 in time");
	else if (ret0 || ret1 || ret2)
		snprintf(cpu_oc_result, sizeof(cpu_oc_result),
			 "FAIL: c0=%d c1=%d c2=%d", ret0, ret1, ret2);
	else
		snprintf(cpu_oc_result, sizeof(cpu_oc_result),
			 "OK: c0=%u c1=%u c2=%u KHz (stock %u/%u/%u)",
			 cpu_c0_target_khz ? cpu_c0_target_khz : g_c0_orig_khz,
			 cpu_c1_target_khz ? cpu_c1_target_khz : g_c1_orig_khz,
			 cpu_c2_target_khz ? cpu_c2_target_khz : g_c2_orig_khz,
			 g_c0_orig_khz, g_c1_orig_khz, g_c2_orig_khz);
...
MODULE_PARM_DESC(cpu_oc_apply, "Write 1 to apply all configured 3-cluster CPU targets to idx0");
...
static const struct kernel_param_ops cpu_c0_min_khz_ops = {
	.set = cpu_c0_min_khz_set, .get = cpu_c0_min_khz_get,
};
module_param_cb(cpu_c0_min_khz, &cpu_c0_min_khz_ops, NULL, 0644);

static int cpu_c1_min_khz_set(const char *val, const struct kernel_param *kp)
{
	return cpu_min_khz_set(cpu_c1_rep_cpu, &g_c1_requested_min_khz, val);
}
static int cpu_c1_min_khz_get(char *buf, const struct kernel_param *kp)
{
	return cpu_min_khz_get(cpu_c1_rep_cpu, g_c1_requested_min_khz, buf);
}
static const struct kernel_param_ops cpu_c1_min_khz_ops = {
	.set = cpu_c1_min_khz_set, .get = cpu_c1_min_khz_get,
};
module_param_cb(cpu_c1_min_khz, &cpu_c1_min_khz_ops, NULL, 0644);
...
static const struct kernel_param_ops cpu_c2_min_khz_ops = {
	.set = cpu_c2_min_khz_set, .get = cpu_c2_min_khz_get,
};
...
static int guardian_thread_fn(void *unused)
{
	while (!kthread_should_stop()) {
		mutex_lock(&oc_lock);
		guardian_check_one(cpu_c0_rep_cpu, cpu_c0_target_khz, g_c0_orig_khz,
				   g_c0_have_orig, g_c0_requested_min_khz);
		guardian_check_one(cpu_c1_rep_cpu, cpu_c1_target_khz, g_c1_orig_khz,
				   g_c1_have_orig, g_c1_requested_min_khz);
		guardian_check_one(cpu_c2_rep_cpu, cpu_c2_target_khz, g_c2_orig_khz,
				   g_c2_have_orig, g_c2_requested_min_khz);
		mutex_unlock(&oc_lock);
...
static int cpu_stats_refresh_set(const char *val, const struct kernel_param *kp)
{
	...
	mutex_lock(&oc_lock);
	ret = do_stats_refresh(which == 1 ? cpu_c0_rep_cpu : (which == 2 ? cpu_c1_rep_cpu : cpu_c2_rep_cpu),
				which == 1 ? cpu_c0_target_khz : (which == 2 ? cpu_c1_target_khz : cpu_c2_target_khz));
	if (ret == -ENOSYS)
		snprintf(cpu_stats_refresh_result, sizeof(cpu_stats_refresh_result),
			 "FAIL: stats symbols not resolved");
	else if (ret == -ENODEV)
		snprintf(cpu_stats_refresh_result, sizeof(cpu_stats_refresh_result),
			 "FAIL: cpu%u not found/online", which == 1 ? cpu_c0_rep_cpu : (which == 2 ? cpu_c1_rep_cpu : cpu_c2_rep_cpu));
	else
		snprintf(cpu_stats_refresh_result, sizeof(cpu_stats_refresh_result),
...
MODULE_PARM_DESC(cpu_stats_refresh, "Write 1/2/3 to rebuild cluster 0/1/2 time_in_state");
...
static int __init oc_mt6789_init(void)
{
	...
	pr_info("oc_mt6789: 3-cluster CPU OC safety cap = stock +%d%%, %uMHz absolute ceiling\n",
...
static void __exit oc_mt6789_exit(void)
{
	...
	gpu_restore_working_table();
	if (g_c1_have_orig)
		patch_cluster_idx0(cpu_c1_rep_cpu, 0, &g_c1_orig_khz, &g_c1_have_orig, true);
	if (g_c0_have_orig)
		patch_cluster_idx0(cpu_c0_rep_cpu, 0, &g_c0_orig_khz, &g_c0_have_orig, true);
	if (g_c2_have_orig)
		patch_cluster_idx0(cpu_c2_rep_cpu, 0, &g_c2_orig_khz, &g_c2_have_orig, true);
	mutex_unlock(&oc_lock);
...
MODULE_DESCRIPTION("3-cluster CPU + GPU OC for MT6789 Helio G99 — POCO M5 rock");
MODULE_VERSION("2.0.0-3CLUSTER");
