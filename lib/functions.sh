...
# cache key for CI workspace restore — must match on repo/branch/root/lto
build_fingerprint() {
    printf '%s|%s|%s|%s|%s' \
...
# resolves ZyC/Custom toolchain, GKI prebuilt, or PATH clang
clang_bin_path() {
...
# true if this clang was built with Polly support
clang_has_polly() {
