...
## MT6789 peak 3+3+2 logical policy split

The stock MT6789 DTB exposes two physical CPUFreq performance domains: CPU0-5 (A55) and CPU6-7 (A76). This build does not invent a third hardware domain. The driver exposes three Linux policies by logically splitting the A55 domain into CPU0-3 and CPU3-5 while keeping the real stock A55 LUT (500–2000 MHz) for both policies; CPU6-7 retain their native 725–2200 MHz A76 table.

The critical performance fix is shared-domain arbitration: the LUT index is ordered high-frequency first, so the shared A55 controller now selects the lower requested index (the higher requested frequency). This prevents one logical A55 policy from accidentally overriding a higher request from the other policy. No frequency above the hardware LUT is advertised.

This is a topology/control correction, not an overclock. Actual peak frequency remains bounded by the hardware LUT: A55 2000 MHz and A76 2200 MHz. Any later OC is a separate experiment and must be validated against actual hardware frequency and thermal behavior.

## Virtual A55 Policy Split

When `VIRTUAL_A55_POLICY_SPLIT=y`, the build stage patches the Rock `mediatek-cpufreq-hw.c` driver to expose separate Linux cpufreq policies for CPU0-3 and CPU3-5 while sharing the stock A55 hardware performance domain. CPU0-3 keeps the native 500–2000 MHz A55 table; CPU3-5 exposes a separate logical 725–2200 MHz Perf table cloned from the native Perf policy and translates requests to the shared A55 hardware; CPU6-7 keeps its native 725–2200 MHz A76 table. The CPU0-3 and CPU3-5 policies are arbitrated against the shared A55 hardware using the safer lower-frequency request (larger descending-LUT index). The DTB must remain the stock two-domain MT6789 mapping: CPU0-5 -> domain 0 and CPU6-7 -> domain 1. Do not create a third hardware performance-domain node; the third cluster is a Linux cpufreq policy only.
